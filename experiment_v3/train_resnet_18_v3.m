...
netMX = dlnetwork(lgraph);
netMX = initialize(netMX);
...
fileID = fopen('metrics_log_resnet_18_v3.txt', 'w');
...
while epoch < numEpochs
    ...
    while hasdata(mbq)
        ...
        [loss,gradients,state] = dlfeval(@modelLoss,netMX,X,T);
        netMX.State = state;
